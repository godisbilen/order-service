...
const create_order = (orderinfo: orderInfo, force = false): Promise<order> => {
    return new Promise(async (resolve, reject) => {
        ...
        if (
            ...
            !(orderinfo.phone_number || orderinfo.email)
        ) {
            return reject(new Error('Insufficient arguments'));
        }

        if (!force) {
            const is_within = await is_within_regions(
                orderinfo.coordinates,
            ).catch((err) => {
                return reject(err);
            });
            ...
            .then((start_time_detailed) => {
                ...
                get_cars({ id: start_time_detailed.car_id })
                    .then((cars) => {
                        if (!cars[0]) {
                            return reject(
                                'Could not get the car for the previous order',
                            );
                        }
                        const car = cars[0];
                        time_between(
                            start_time_detailed.previous_order
                                ? start_time_detailed.previous_order.location
                                      .coordinates
                                : car.start_pos.coordinates,
                            orderinfo.coordinates,
                        )
                            .then((time_between) => {
                                const arrival_time = dayjs(start_time)
                                    .add(time_between, 'second')
                                    .toDate();
                                ...
                                Car.findByIdAndUpdate(
                                    ...
                                    .then((temp) => {
                                        ...
                                        resolve(
                                            car.orders[car.orders.length - 1],
                                        );
                                    ...
                                    .catch(() => {
                                        return reject(
                                            new Error(
                                                'Could not update car with order',
                                            ),
                                        );
                                    });
                            })
                            .catch(() => {
                                return reject(
                                    new Error(
                                        'Could not get time between previous order and this order',
                                    ),
                                );
                            });
                    })
                    .catch(() => {
                        return reject(
                            'Could not get the car from the database',
                        );
                    });
            })
            .catch(() => {
