...
router.post('/', (req, res) => {
    ...
    if (!data.coordinates || !(data.phone_number || data.email)) {
        return res.status(400).json({
            status: 'Bad Request',
            message: 'One or more required fields was not set',
        });
    }
    ...
    if (!isLngLat(data.coordinates)) {
        return res.status(400).json({
            status: 'Bad Request',
            message: "Field 'coordinates' is not a valid LngLat object",
        });
    }
    if (
    ...
    if (data.hasOwnProperty('from')) {
        if (!dayjs(data.from).isValid()) {
            return res.status(400).json({
                status: 'Bad Request',
                message: "Field 'from' is not a valid date value",
            });
        }
        if (validator.isBefore(data.from, dayjs().toISOString())) {
            return res.status(400).json({
                status: 'Bad Request',
                message: "Field 'from' is not an upcomming date",
            });
        }
    }
    create_order(data)
