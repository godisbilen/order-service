...
router.post('/', (req, res) => {
    ...
    removeKeysExcept(data, ['nickname', 'driver', 'region', 'start_time', 'start_pos']);

    if (!data.hasOwnProperty('nickname') || !validator.isLength(data.nickname, { min: 4 })) {
    ...
    if (data.hasOwnProperty('driver') && !validator.isMongoId(data.driver)) {
        return res
            .status(400)
            .json({ status: 'Bad Request', message: "Field 'driver' is not an valid MongoDB ObjectID" });
    }
    if (data.hasOwnProperty('region') && !validator.isMongoId(data.region)) {
        return res
            .status(400)
            .json({ status: 'Bad Request', message: "Field 'region' is not aa valid MongoDB ObjectID" });
    }
    if (data.hasOwnProperty('start_time') && !validator.isNumeric('' + data.start_time)) {
        return res
            .status(400)
    ...
    if (!data.hasOwnProperty('start_pos') || !isLngLat(data.start_pos)) {
        return res
            .status(400)
            .json({ status: 'Bad Request', message: "Field 'start_pos' is not a valid LngLat value" });
    }
