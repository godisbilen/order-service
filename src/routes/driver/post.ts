...
router.post('/', (req, res) => {
    ...
    if (!data.hasOwnProperty('username') || !validator.isLength(data.username, { min: 6 })) {
        return res
            .status(400)
            .json({ status: 'Bad Request', message: "Field 'username' is required and minimun length of 6" });
    }
    if (!data.hasOwnProperty('firstname') || !validator.isLength(data.firstname, { min: 2 })) {
        return res
            .status(400)
            .json({ status: 'Bad Request', message: "Field 'firstname' is required and minimun length of 2" });
    }
    if (!data.hasOwnProperty('lastname') || !validator.isLength(data.lastname, { min: 2 })) {
        return res
            .status(400)
            .json({ status: 'Bad Request', message: "Field 'lastname' is required and minimun length of 2" });
    }
