...
router.get('/', (req, res) => {
    ...
    removeKeysExcept(filter, ['id', 'username', 'firstname', 'lastname', 'car']);
    ...
    if (filter.hasOwnProperty('username') && !validator.isLength(filter.username, { min: 6 })) {
        return res
            .status(400)
            .json({ status: 'Bad Request', message: "Field 'username' needs to have a minimun length of 6" });
    }
    if (filter.hasOwnProperty('firstname') && !validator.isLength(filter.firstname, { min: 2 })) {
        return res
            .status(400)
            .json({ status: 'Bad Request', message: "Field 'firstname' needs to have a minimun length of 2" });
    }
    if (filter.hasOwnProperty('lastname') && !validator.isLength(filter.lastname, { min: 2 })) {
        return res
            .status(400)
            .json({ status: 'Bad Request', message: "Field 'lastname' needs to have a minimun length of 2" });
    }
    if (filter.hasOwnProperty('car') && !validator.isMongoId(filter.car)) {
        return res.status(400).json({ status: 'Bad Request', message: "Field 'car' is not a valid MongoDB ObjectID" });
    }
